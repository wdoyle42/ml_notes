...
We'll split the data into training and testing, using the default 75/25 split. Try this
...
```{r}
ols_coeffs<-cr_wf%>%extract_fit_parsnip()%>%tidy()
```
...
```{r}
enet_grid<-grid_regular(extract_parameter_set_dials(cr_tune_model) ,levels=10)
```
...
```{r}
best_enet<-select_best(cr_enet_tune_fit)

best_enet_penalty<-bestenety$penalty[1]
...
cr_enet_tune_fit%>%show_best()
cr_enet_tune_fit%>%select_best()
=======
